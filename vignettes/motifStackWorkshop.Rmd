...
Motifs are often represented as position count matrix (PCM)(Table \ref{motifstacktbl:PCM}), position frequency matrix (PFM)(Table \ref{motifstacktbl:PFM})
...
kable(pcm, caption = "\\label{motifstacktbl:PCM}Sample of PCM. Each row represents the counts of A, C, G and T, while each column is the counts for that nucleotide position.")
```
...
kable(pfm, digits=2,
      caption = "\\label{motifstacktbl:PFM}Sample of PFM. Each row represents the frequency of A, C, G and T, while each column is the frequency for that nucleotide position.")
```
