...
Here we clustered motifs for a smaller set of fly Homeodomain(HDs) that were determined using four different experimental methods to measure DNA binding specificity, B1H, HT-SELEX, SMiLE-seq and PBM, to confirm that the motif generation method can impact clustering. As it shown in Figre \ref{fig:Flydata} B1H motifs from different lab tend to cluster with each other (motifs 1,2, 37,38, 41-113, 132,133, 136-162, 166-171, 180-195, 206-236, 242,243) compare to PBM data (motifs 'eve' in blue). PMB data (motif 5-13, 18-21, 23-26) tend to have lower IC, which may be introduced by computation methods[@ou2018motifstack]. HT-SELEX tend to have dimer (motif labels in red). 

```{r, fig.width=12, fig.height=12, fig.cap="\\label{fig:Flydata}Fly data from different methods."}
